...
def get_list():
    err_msg = ""
    if request.method.__eq__('POST'):
        id_stu = request.form.get('id_stu')
        name = request.form.get('name')
        ...
        try:
            utils.add_students(id_stu=id_stu, name=name, sex=sex,  birthday=birthday, address=address, number=number, email=email)
...
def get_scores():
    ...
    if request.method.__eq__('POST'):
        ...
        try:
            utils.add_students(name=name, diem15=diem15, diem1t=diem1t, diemthi=diemthi)
